...
from utils import plot_loss_from_mtx
...
def run_train_MLP(epochs=30, channels=3, resize_value=128, batch_size=8, hidden_layers=2, output_path='weights/MLP', dataset_path='dataset', show=False, to_save=True):
	...
	dataset = DatasetLoader(dataset_path=dataset_path, resize_value=resize_value)
...
def run_train_GNN(epochs=30,resize_value=64, batch_size=8, n_blocks=2, max_samples=None, output_path='weights/GNN',dataset_path='data/mnist/test', patience=5, grayscale=False):
	...
	from datasets import GraphDatasetLoader
	
	# Use optimized dataset loader with caching 
	original_dataset = GraphDatasetLoader(
